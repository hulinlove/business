<template>
  <div>
    <myHeader head-title="修改地址" goBack="true"></myHeader>
    <div class="wrap">
      <ul class="addressDetails">
        <li>
          <span>联系人</span>
          <input v-model.trim="receiverName">
        </li>
        <li>
          <span>手机号码</span>
      ...
      <div v-if="showSelect">
        <v-distpicker type="mobile" :province="province.value" :city="city.value" :area="area.value" @province="onChangeProvince" @city="onChangeCity" @area="onChangeArea"></v-distpicker>
      </div>
...
<script>
import {editAddress, saveAddress} from '../../../service/getData'
...
export default {
  ...
  methods: {
    // 初始化
    async init () {
      await editAddress({addressId: this.addressId}).then(res => {
        if (res.code === 0) {
    ...
    async save () {
      ...
      await saveAddress({address: address}).then(res => {
